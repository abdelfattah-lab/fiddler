...
class FiddlerMixtralWithPredictor:
    ...
    def mixtral_forward(self, input_ids, position_ids, is_decode):
        ...
        for i_layer, layer in enumerate(self.model.layers):
            ...
            # Start predictor after layer 0 completes
            if i_layer == 0 and self.cpu_offload != 0:
                self.prediction_thread = threading.Thread(
                    target=self.dummy_predictor,
                    args=(input_ids, position_ids, 2)
                )
            ...
            if self.cpu_offload == 0:
                ...
                # For layers 2+, use predictor results if available
                predicted_config = None
                if i_layer >= 2:
                    with self.prediction_lock:
                        predicted_config = self.prediction_results.get(i_layer, None)
                
                if predicted_config is not None and predicted_config != -1:
                    # Use predicted configuration
                    best_config = predicted_config
                else:
    ...
    def dummy_predictor(self, input_ids, position_ids, start_layer=2):
        """Dummy predictor that generates random expert predictions for layers 2+"""
        try:
            # For each token position, randomly predict which experts will be needed
            ...
            # Generate predictions for layers 2 and above
            for i_layer in range(start_layer, self.n_layer):
                # For each token, randomly select 2 experts (similar to actual routing)
                layer_predictions = []
                for _ in range(num_tokens):
                    # Randomly select 2 experts for this token
                    selected_experts = random.sample(range(self.n_expert), 2)
                    layer_predictions.append(selected_experts)
                
                # Convert to tensor format similar to actual selected_experts
                predicted_experts = torch.tensor(layer_predictions, device=self.dev)
                
                # Calculate dummy best_config based on predictions
                expert_mask = torch.nn.functional.one_hot(
                    predicted_experts, num_classes=8
                ).permute(2, 1, 0)
                
                cost_per_expert = np.zeros((8, 2), dtype=float)  # 0: CPU, 1: GPU
                for i_expert in range(8):
                    idx, top_2 = torch.where(expert_mask[i_expert])
                    # Use same cost calculation as baseline
                    cost_per_expert[i_expert, 0] = top_2.shape[0] * self.latency_cpu
                    cost_per_expert[i_expert, 1] = self.latency_gpu
                    if self.is_expert_in_gpu(i_layer, i_expert):
                        cost_per_expert[i_expert, 1] = 0
                
                # Calculate best config using same algorithm as baseline
                best_config = -1
                best_cost = float("inf")
                ...
                predictions[i_layer] = best_config
            
            # Store predictions
            with self.prediction_lock:
                self.prediction_results.update(predictions)
                
        except Exception as e:
            print(f"Predictor error: {e}")
            # On error, store empty predictions
            with self.prediction_lock:
                for i_layer in range(start_layer, self.n_layer):
                    self.prediction_results[i_layer] = -1
