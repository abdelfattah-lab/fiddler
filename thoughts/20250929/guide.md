...
Plan the implementation of a predictor that would be able to predict which experts would be used for each token in the upcoming layers based on the output of the first layer attention. Let's assume the predictor would be a simple MLP with 2 layers. The input to the MLP would be the output of the first layer attention block for each token. The output would be a score for each of the 60 experts of each of the layers from 2 to 27. The MLP would be trained to minimize the cross entropy loss between the predicted scores and the actual experts scores generated by the gating mechanism. The training data would be generated by running the model on a large corpus of text and recording the output of the first layer attention and the experts scores for each token. The training would be done offline and the trained model would be used during inference to predict the experts for each token in the upcoming layers. The predicted experts would then be prefetched to the GPU memory before they are needed.
Your output should be a detailed plan of how to implement this predictor, including the data collection, model architecture, training procedure, and integration with the existing Fiddler system. I will review this plan then the finalized plan should be enough for another agent to do that implementation and verify that its working.
