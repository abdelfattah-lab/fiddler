# Fiddler MoE Optimization Project

## Project Overview

**Goal**: Optimize MoE (Mixture of Experts) inference by prefetching experts from CPU to GPU
**Model**: Qwen2.5-7B-Instruct (24 MoE layers, 60 experts per layer)
**Best Result**: 3.21x speedup using 7-expert prefetching (0.679s vs 2.182s baseline)

## Key Files

- **`src/fiddler/qwen.py`** - Baseline: on-demand CPU→GPU loading with pinned memory
- **`src/fiddler/qwen_with_prefetch.py`** - Prefetch system (configurable 0-16 experts)
- **`benchmark_prefetch_configs.py`** - Tests all prefetch configurations (0-16)
- **`getting_started.md`** - Complete setup guide for new machines
...
# Quick test (prefetch - runs twice: collection then prediction)
python quick_test.py FiddlerQwenWithPrefetch

# Full benchmark (all configs 0-16 experts)
python benchmark_prefetch_configs.py

# Profile with Nsight
nsys profile --trace=cuda,nvtx,osrt --cuda-memory-usage=true python <script.py>
```

## Important Findings

- **Prefetch sweet spot**: 7 experts (4.285x vs old baseline, 3.21x vs pinned baseline)
- **Pinned memory**: 1.33x speedup over non-pinned baseline
- **Fiddler mode**: CPU execution provides negligible benefit (1-2%), GPU prefetch is superior
- **GPU-resident layers**: Layers 0-1 kept permanently on GPU (can't be prefetched by layer N→N+2)
...
**Completed Tasks**:
1. ✅ Added `all_gpu_mode` parameter to `FiddlerQwenWithPrefetch`
2. ✅ Implemented `_load_all_experts_on_gpu()` method to load all 24×60=1440 experts on GPU
3. ✅ Created comprehensive benchmark comparing all modes
4. ✅ Tested correctness of all-GPU mode

**Key Findings**:
- **All-GPU mode achieves 3.74x speedup** vs baseline (0.431s vs 1.612s)
- All-GPU mode is the **fastest configuration** overall
- Prefetch (16 experts) achieves 2.47x speedup (0.654s)
- Prefetch (4 experts) achieves 1.90x speedup with high variance
- Prefetch (7 experts) shows 1.70x speedup with high variance
- Prefetch (0 experts) matches baseline (0.99x)

**Benchmark Results** (Qwen/Qwen1.5-MoE-A2.7B, 20 tokens, 5 runs):

| Configuration | Avg Time (s) | Speedup vs Baseline | Notes |
|--------------|--------------|---------------------|-------|
| Baseline (on-demand) | 1.612 ± 0.002 | 1.00x | CPU→GPU on-demand loading |
| All-GPU mode | **0.431 ± 0.001** | **3.74x** | All experts on GPU (fastest) |
| Prefetch (0 experts) | 1.626 ± 0.001 | 0.99x | Matches baseline |
| Prefetch (4 experts) | 0.850 ± 0.423 | 1.90x | High variance in output |
| Prefetch (7 experts) | 0.951 ± 0.419 | 1.70x | High variance in output |
| Prefetch (16 experts) | 0.654 ± 0.001 | 2.47x | Stable performance |

**Important Observation**: Prefetch modes (4 and 7 experts) showed **high variance** in both timing and output quality. Some runs generated incorrect Chinese characters instead of "Paris", suggesting the prefetch predictor may not be working optimally or the cached patterns are stale. The 16-expert prefetch was more stable but still slower than all-GPU mode.

**Conclusion**: For systems with sufficient GPU memory, **all-GPU mode is the best choice** (3.74x speedup). If GPU memory is limited, prefetch with 16 experts provides 2.47x speedup as a good middle ground.

**Files Created**:
- `test_all_gpu_mode.py` - Quick test for all-GPU mode
- `benchmark_all_gpu_vs_prefetch.py` - Comprehensive benchmark script
- `all_gpu_benchmark_1759427429/` - Results directory with CSV, JSON, plot

**Files Modified**:
- `src/fiddler/qwen_with_prefetch.py` - Added all-GPU mode support

## Guidelines

Update guide.md after each session. Stage all relevant changed files and suggest commit message.
