...
Please inspect MixtralWithPrefetch. I believe the prefetching is not truly asynchronous with the computation and we need to fix that. Can you please make sure they are asynchronous by adding some profiling or debugging statements that verify whether they're asynchronous or not and if not asynchronous, make them asynchronous.
...
- `profile_nsight.py` - **Unified profiling script** ⭐
...
- `profile_nsight.py` - **Unified profiling script** (replaces separate scripts)
  - Usage: `python profile_nsight.py baseline-vs-prefetch`
  - Usage: `python profile_nsight.py hit-rate-comparison`
