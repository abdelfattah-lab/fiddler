...
Let's modify the Nsight profiling script so that it creates its output in the following structure: profiling_results/{timestamp}/{name}
then let's delete the expert_usage_patterns.json and run the profiling to know the behavior when there's no prefetching. Let's give it the name = no_prefetch
then let's run the profiling again (the expert_usage_patterns.json would have been generated by the last run) to understand the bahavior when prefetching is active. Let's give it the name = prefetch
Please double check that in each profiling run, the hit rate matches what we expect and report if something is unexpected.
